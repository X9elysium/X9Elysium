...
// Voice presets — match by browser SpeechSynthesisVoice name + lang regex.
// The browser ships these via the OS speech engines (macOS, Windows, Chromium).
// All presets are female voices: 4 British, 4 Indian. No API key required.
type Preset = {
  id: string;
  label: string;
  flag: string;
  accent: "British" | "Indian";
  // Regex against `${voice.name} ${voice.lang}`. Tried in order — first match wins.
  patterns: string[];
  pitch?: number;
  rate?: number;
};
...
const PRESETS: Preset[] = [
  // British — 4 options. Most macOS/iOS users get Kate, Serena, Lily, Martha.
  // Windows ships Hazel; Chrome on any OS ships "Google UK English Female".
  {
    id: "british-kate",
  ...
  // Indian — 4 options. macOS ships Veena (en-IN). Windows ships Heera. Chrome
  // ships "Google हिन्दी" + "Google English (India)".
  {
    id: "indian-veena",
  ...
  },
  {
    ...
    flag: "🇮🇳",
    accent: "Indian",
    patterns: ["Microsoft Neerja", "Google हिन्दी", "Veena.*en[-_]IN", "en[-_]IN"],
    pitch: 1.08,
  },
  {
    id: "indian-priya",
...
function chunkText(text: string): string[] {
  // Chrome stops single utterances after ~15s, so split on sentences (~280 chars).
  const sents = text
    .replace(/\n{2,}/g, ". ")
    .replace(/\n/g, " ")
    .split(/(?<=[.!?])\s+/)
...
export default function Player({ speakableText, title }: Props) {
  ...
  function estimateTotalSec(text: string, speed: number) {
  ...
  function changePreset(id: string) {
    ...
    if (state === "playing" || state === "paused") {
      reset();
      // Re-trigger play after reset commits
      setTimeout(() => {
        setPresetId(id);
        play();
      }, 50);
  ...
  return (
    <div className="grok-player relative">
      <div className="flex items-center gap-3 rounded-2xl border border-neutral-200/70 dark:border-white/[0.08] bg-white/80 dark:bg-white/[0.03] backdrop-blur-sm p-3 lg:p-4 shadow-sm">
        {/* Play / Pause */}
        <button
          type="button"
          onClick={onTogglePlay}
          aria-label={state === "playing" ? "Pause" : "Play"}
          className="grok-play-btn shrink-0 w-11 h-11 lg:w-12 lg:h-12 rounded-full bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 grid place-items-center hover:scale-[1.04] active:scale-95 transition-transform"
        >
          {state === "loading" ? (
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <p className="text-[11px] font-mono uppercase tracking-[0.18em] text-neutral-500 dark:text-neutral-400 truncate">
              Listen · {preset.flag} {preset.label}
            </p>
          </div>
